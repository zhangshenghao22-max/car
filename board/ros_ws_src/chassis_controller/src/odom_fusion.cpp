...
public:
  ...
  : Node("odom_fusion")
  {
    ...
    this->declare_parameter<double>("imu_dt_warn_threshold", 0.2);  // Warn/adapt only when IMU period is above this threshold (s)
    ...
    imu_dt_warn_threshold_ = this->get_parameter("imu_dt_warn_threshold").as_double();
    if (imu_dt_warn_threshold_ <= 0.0) {
      RCLCPP_WARN(this->get_logger(), "imu_dt_warn_threshold <= 0, resetting to 0.2s");
      imu_dt_warn_threshold_ = 0.2;
    }
...
private:
  void imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg)
  {
    ...
    if (has_received_imu_) {
      ...
      if (dt > imu_dt_warn_threshold_) {
        // Yahboom IMU normally runs near 10Hz, so the default warning threshold is 0.2s.
        adaptive_threshold = max_yaw_rate_ * imu_dt_warn_threshold_ / dt;
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                    "Large IMU dt detected (%.6f s, threshold %.3f s), using adaptive yaw_rate threshold: %.2f rad/s",
                    dt, imu_dt_warn_threshold_, adaptive_threshold);
      }
