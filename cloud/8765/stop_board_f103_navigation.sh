...
if [[ -f "${ROOT_DIR}/board_nav_command.py" ]]; then
  python3 "${ROOT_DIR}/board_nav_command.py" stop-cruise >/dev/null 2>&1 || true
fi
...
pkill -TERM -f "rt_robot_nav2_complete.launch.py.*use_nav:=true" >/dev/null 2>&1 || true
