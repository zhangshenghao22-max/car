...
private:
    ...
    // Startup delay to wait for sensor data to stabilize
    double startup_delay_;
    rclcpp::Time startup_time_;
    
    // Map boundary expansion validation
    // Track previous robot pose to detect if robot actually moved (not just rotated)
    GMapping::OrientedPoint last_robot_pose_;
    bool has_last_robot_pose_;
    // Minimum linear movement required for boundary expansion (meters)
    // This prevents boundary expansion when robot only rotates near boundary
    double min_linear_movement_for_expansion_;
    // Maximum boundary expansion per update (meters)
    // This limits how fast the map can expand to prevent errors
    double max_expansion_per_update_;
};
