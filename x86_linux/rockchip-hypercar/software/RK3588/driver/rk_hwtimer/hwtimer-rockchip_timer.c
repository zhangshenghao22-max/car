...
#ifdef RT_USING_KTIME
static struct rk_timer _timer0 = {0};
...
{
    return rk_timer_current_value(&_timer0);
}

rt_err_t rt_ktime_hrtimer_settimeout(uint64_t cnt)
{   
    struct rk_timer *time = &_timer0;
    rt_base_t            level;
...
INIT_DEVICE_EXPORT(rk_timer_drv_register);
