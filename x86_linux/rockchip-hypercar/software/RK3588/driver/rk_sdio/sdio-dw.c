...
#define DIV64_ROUND_DOWN_ULL(ll, d)    ({ rt_uint64_t _tmp = (ll); _do_div64(_tmp, d); _tmp; })
#define DIV64_ROUND_UP_ULL(ll, d)      DIV64_ROUND_DOWN_ULL((rt_uint64_t)(ll) + (d) - 1, (d))
...
rt_inline void sdio_dw_set_cto(struct sdio_dw *sd)
{
    ...
    cto_ms = DIV64_ROUND_UP_ULL((rt_uint64_t)MSEC_PER_SEC * cto_clks * cto_div,
            sd->bus_hz);
...
static void sdio_dw_set_data_timeout(struct sdio_dw *sd, rt_uint32_t timeout_ns)
{
    ...
    tmp = DIV64_ROUND_UP_ULL((rt_uint64_t)timeout_ns * sd->bus_hz, NSEC_PER_SEC);
    tmp = DIV64_ROUND_UP_ULL(tmp, clk_div);
...
#ifdef SD_WITCH_VOLTAGE_SUPPORT
...
#ifdef SD_MMC_CARD_BUSY_SUPPORT
...
static void sdio_dw_set_drto(struct sdio_dw *sd)
{
    ...
    drto_ms = DIV64_ROUND_UP_ULL((rt_uint64_t)MSEC_PER_SEC * drto_clks * drto_div,
            sd->bus_hz);
...
static rt_err_t sdio_dw_init_slot(struct sdio_dw *sd)
{
    ...
    if (err)
    {
        LOG_I("%s : sdio_regulator_get_supply err = %d", __func__, err);
        //goto _free;
    }
#endif
...
rt_err_t sdio_dw_probe(struct sdio_dw *sd)
{
    ...
    if (!sd->biu_clk || !sd->ciu_clk)
    {
        ...
        LOG_I("%s : get clk biu or ciu error", __func__);
        err = -RT_EIO;
        //goto _free_res;
    }
...
#ifdef RT_USING_PIC
    rt_pic_attach_irq(sd->irq, sdio_dw_isr, sd, dev_name, RT_IRQ_F_NONE);
    rt_pic_irq_unmask(sd->irq);
#else
